const inputAddTask: HTMLElement = document.querySelector('.to-do-list__input_type_add-task');
const buttonSubmitAddTask:  HTMLElement = document.querySelector('.to-do-list__button_type_submit-add-task');
const formTaskList:  HTMLElement = document.querySelector('.to-do-list__form-task-list');
const listTaskItems:  NodeList = document.querySelectorAll('.to-do-list__task-item');
const taskList:  HTMLElement = document.querySelector('.to-do-list__task-list');

const taskItemTemplate: HTMLElement = document.querySelector('#task-item-template');
...
let tasks: (TaskItemConfig)[] = [
  { id: 1, text:' Полить цветы'  },
  { id: 2, text: 'Сходить в магазин' },
  { id: 3, text: 'Выбрать монитор на Яндекс Маркете' },
];

let valueInputAddTask: string = '';

const handleSettingTaskItem = ({text, id}: TaskItemConfig) : HTMLElement => {
  const taskItem: HTMLTemplateElement = taskItemTemplateContent.querySelector('.to-do-list__task-item').cloneNode(true);
  const taskItemInput: HTMLInputElement = taskItem.querySelector('.to-do-list__input_type_task-item');
  const taskItemNumber: HTMLElement = taskItem.querySelector('.to-do-list__task-item-number');
  const taskItemEditButton: HTMLElement = taskItem.querySelector('.to-do-list__task-item-button_type_edit');
  const taskItemDeleteButton: HTMLElement = taskItem.querySelector('.to-do-list__task-item-button_type_delete');
...
buttonSubmitAddTask.addEventListener('click', (event) => {
  ...
  const inputAddTask = event.target as HTMLInputElement;
